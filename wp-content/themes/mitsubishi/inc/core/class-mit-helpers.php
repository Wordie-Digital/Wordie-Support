...
class MIT_Helpers {
  ...
  function get_personalised_field( $selector, $post_id = false ) {
    $user_type = $this->get_current_user_type();

    if ( ! empty( $value = get_field( "{$selector}_$user_type", $post_id ) ) ) {
      return $value;
    }

    // Fallback;
    return get_field( $selector, $post_id ) ?: get_field( "{$selector}_" . MIT_USER_TYPE__HO, $post_id );
  }

  function get_related_posts( $post_id, $related_count, $post_type = 'post' ) {
  ...
  function get_current_user_type(): string {
    if (
      isset( $_COOKIE[ MIT_USER_TYPE__COOKIE ] ) &&
      in_array( $_COOKIE[ MIT_USER_TYPE__COOKIE ], MIT_USER_TYPES )
    ) {
      return sanitize_key( $_COOKIE[ MIT_USER_TYPE__COOKIE ] );
    }

    return MIT_USER_TYPE__HO;
  }
  ...
  function get_home_url_switch( $user_type ) {
    return home_url( MIT_HOME_SLUGS[ $user_type ] . '?' . MIT_USER_TYPE__GET_PARAM . '=' . $user_type );
  }

  function get_portal_home_url() {
    switch_to_blog( MIT_PORTAL_BLOG_ID );
    $home_url = home_url();
  ...
  function is_post_in_descendant_category( $term_ids, $_post = null, $taxonomy = 'category' ): bool {
  ...
  function is_flow_homeowner(): bool {
    return MIT_USER_TYPE__HO == $this->get_current_user_type();
  }

  function is_flow_professional(): bool {
    return MIT_USER_TYPE__PRO == $this->get_current_user_type();
  }
  ...
  /*function is_product_cat_belongs_to_industrial( $product_cat_id ): bool {
    $industrial_cat = get_term_by( 'slug', 'industrial', 'product_cat' );
    $ancestors      = get_ancestors( $product_cat_id, 'product_cat', 'taxonomy' );

    if (
      $industrial_cat &&
      ! is_wp_error( $industrial_cat ) ) {
      return
        (
          ! in_array( $product_cat_id, [ // TODO: Remove this hardcode, add a switch toggle in the category edit page
            get_term_by( 'slug', 'factory-automation', 'product_cat' )->term_id ?? 0,
            get_term_by( 'slug', 'automation-software', 'product_cat' )->term_id ?? 0,
            get_term_by( 'slug', 'switchgear', 'product_cat' )->term_id ?? 0,
            get_term_by( 'slug', 'ups-lithium-ion-battery-solutions', 'product_cat' )->term_id ?? 0,
          ] ) && (
            $product_cat_id == $industrial_cat->term_id ||
            in_array( $industrial_cat->term_id, $ancestors )
          )
        );
    }
  ...
  function is_product_cat_belongs_to_industrial( $product_cat_id ): bool {
    ...
    if ( $industrial_cat && ! is_wp_error( $industrial_cat ) ) {

        // Get the term ID for factory-automation
        $factory_automation_cat = get_term_by( 'slug', 'factory-automation', 'product_cat' );

        // Check if factory-automation category exists
        if ( $factory_automation_cat && ! is_wp_error( $factory_automation_cat ) ) {
            // Get all child categories of factory-automation
            $child_categories = get_term_children( $factory_automation_cat->term_id, 'product_cat' );

            // Add the factory-automation and its child categories to the exclusion list
            $exclude_ids = array_merge(
                [ 
                    $factory_automation_cat->term_id,
                    get_term_by( 'slug', 'automation-software', 'product_cat' )->term_id ?? 0,
                    get_term_by( 'slug', 'switchgear', 'product_cat' )->term_id ?? 0,
                    get_term_by( 'slug', 'ups-lithium-ion-battery-solutions', 'product_cat' )->term_id ?? 0,
                ],
                $child_categories // Include children categories
            );

            // Check if the category ID is in the exclusion list
            if ( ! in_array( $product_cat_id, $exclude_ids ) ) {
                // Proceed if the category is in the industrial category or its ancestors
                return ( 
                    $product_cat_id == $industrial_cat->term_id || 
                    in_array( $industrial_cat->term_id, $ancestors )
                );
